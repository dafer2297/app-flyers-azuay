...
# REGLA TIPO 9 (Alto max 300, Ancho max 600)
...
def generar_tipo_2_v1(datos):
    ...
    # 3. Descripcion 2 (Izquierda, sobre la caja)
...
def generar_tipo_2_v2(datos):
    ...
    # 2. Calculo Caja (Apilada a 100px sobre ubicación)
    GAP_LOC_BOX = 100
    h_caja = 548
    w_caja = 548
    x_box = SIDE_MARGIN
    ...
    # 4. Desc2 (Izquierda, sobre la caja de fecha)
    desc2 = datos.get('desc2', "")
    s_desc2 = 68
    f_desc2 = get_font("Canaro-Medium.ttf", s_desc2)
    ...
    # Dibujar Desc2 (Izquierda, sobre la caja de fecha)
    if desc2:
        y_cursor_d2 = y_desc2_top + int(s_desc2 * 1.1)
        for l in lines_d2:
            dibujar_texto_sombra(draw, l, x_box, y_cursor_d2, f_desc2, offset=(4,4), anchor="ls")
...
def generar_tipo_2_v2(datos):
    ...
    # 4. Desc2 (Izquierda, sobre la caja de fecha) <-- CORREGIDO AQUI
    desc2 = datos.get('desc2', "")
    s_desc2 = 68
    f_desc2 = get_font("Canaro-Medium.ttf", s_desc2)
    lines_d2 = wrap_text_pixel(desc2, f_desc2, 800)
...
def generar_tipo_2_v3(datos):
    ...
    # 3. Descripcion 1 (DINAMICA, Izquierda) - BAJADO 30PX
    limit_y = min(y_desc2_top, y_loc_top) - 50 # Limita con Desc2 o Ubicación
    y_start_desc1 = y_titulo + 180 
    max_h_desc1 = limit_y - y_start_desc1
...
def generar_tipo_2_v4(datos):
    ...
    limit_y = min(y_desc2_top, y_firma_top) - 50 # <-- AHORA LIMITA CON LA DESC2 O FIRMA
    y_start_desc1 = y_titulo + 180 
    max_h_desc1 = limit_y - y_start_desc1
    ...
    # Dibujar Desc2 (Sobre la caja a la izquierda)
    if desc2:
        y_cursor_d2 = y_desc2_top + int(s_desc2 * 1.1)
        for l in lines_d2:
            dibujar_texto_sombra(draw, l, x_box, y_cursor_d2, f_desc2, offset=(4,4), anchor="ls")
...
def generar_tipo_3_v1(datos):
    ...
    # 2. Caja Larga Fecha (Izquierda, Ancho Dinámico)
    ...
    # HEREDANDO DISTANCIA BASE EXACTA DEL CUADRADO (- 72 - hcaja)
    ...
    # Dibujar Caja Larga (Estirada dinámicamente)
...
def generar_tipo_3_v2(datos):
    ...
    # 2. Caja Larga Fecha (Izquierda, Ancho Dinámico, Apilada 100px)
    dia1 = datos['fecha1'].day
    dia2 = datos['fecha2'].day if datos['fecha2'] else dia1
    mes_nombre = obtener_mes_nombre(datos['fecha1'].month)
    texto_dias = f"{dia1} al {dia2}"
...
def generar_tipo_4_v2(datos):
    ...
    # 3. Descripcion 2 (Izquierda, Apilada sobre la caja de fecha) <-- CORRECCIÓN APLICADA
    desc2 = datos.get('desc2', "")
    s_desc2 = 68
    f_desc2 = get_font("Canaro-Medium.ttf", s_desc2)
    lines_d2 = wrap_text_pixel(desc2, f_desc2, 800)
...
def generar_tipo_4_v4(datos):
    ...
    # 3. Descripcion 2 (Izquierda, apilada sobre la caja de fecha) <-- CORRECCION
    desc2 = datos.get('desc2', "")
    s_desc2 = 68
    f_desc2 = get_font("Canaro-Medium.ttf", s_desc2)
    lines_d2 = wrap_text_pixel(desc2, f_desc2, 800)
    ...
    limit_y = min(y_desc2_top, y_firma_top) - 50 # <-- AHORA LIMITA CON DESC2 O FIRMA
    y_start_desc1 = y_titulo + 180 # <-- CORRECCION 30px
    max_h_desc1 = limit_y - y_start_desc1
...
def generar_tipo_5_v1(datos):
    ...
    # 1. Calculo Exacto Ubicacion (Izquierda) <-- INTERCAMBIO AQUI
    lugar = datos['lugar']
    s_lug = 61 if len(lugar) < 45 else 51 
    f_lugar = get_font("Canaro-Medium.ttf", s_lug)
    lines_loc = wrap_text_pixel(lugar, f_lugar, 800)
    ...
    # 2. Caja Fecha (Izquierda, Apilada 100px sobre la ubicacion) <-- INTERCAMBIO AQUI
    GAP_LOC_BOX = 100
    h_caja = 548
    w_caja = 548
    x_box = SIDE_MARGIN
...
def generar_tipo_5_v3(datos):
    ...
    # 2. Caja Fecha (Izquierda, Apilada 100px sobre ubicacion) <-- INTERCAMBIO AQUI
    GAP_LOC_BOX = 100
    h_caja = 548
    w_caja = 548
    x_box = SIDE_MARGIN
    ...
    # 3. Logo Colaborador (Derecha Abajo) <-- INTERCAMBIO AQUI
    collab_img = None
    y_logo_collab_top = Y_BOTTOM_BASELINE
    if datos.get('logos') and len(datos['logos']) > 0:
        try:
...
def generar_tipo_6_v1(datos):
    ...
    # 1. LOGICA DE DISTRIBUCION DE LOS 3 LOGOS SUPERIORES (REGLA 600x300)
    collab_img = None
    if datos.get('logos') and len(datos['logos']) > 0:
        try:
            collab_img = redimensionar_logo_colaborador_top(Image.open(datos['logos'][0]).convert("RGBA"))
        except Exception as e: pass
...
def generar_tipo_6_v2(datos):
    ...
    # 4. Desc2 (Izquierda, SIEMPRE sobre la caja de fecha) <-- CORRECCION
    desc2 = datos.get('desc2', "")
    s_desc2 = 68
    f_desc2 = get_font("Canaro-Medium.ttf", s_desc2)
...
def generar_tipo_6_v3(datos):
    ...
    # 1. 3 LOGOS SUPERIORES (NUEVA REGLA TOPE 600x300)
    collab_img = None
    if datos.get('logos') and len(datos['logos']) > 0:
        try:
            collab_img = redimensionar_logo_colaborador_top(Image.open(datos['logos'][0]).convert("RGBA"))
    ...
    # 3. Descripcion 1 (DINAMICA, Izquierda) - BAJADO 30PX Y LIMITE EXACTO
    limit_y = min(y_desc2_top, y_loc_top) - 50
...
def generar_tipo_6_v4(datos):
    ...
    # 4. Desc2 (Izquierda, sobre la caja fecha) <-- CORRECCION
    desc2 = datos.get('desc2', "")
    s_desc2 = 68
    f_desc2 = get_font("Canaro-Medium.ttf", s_desc2)
    lines_d2 = wrap_text_pixel(desc2, f_desc2, 900)
...
def generar_tipo_7_v1(datos):
    ...
    # 2. Caja Fecha Larga (Izquierda, Ancho Dinamico, Apilada sobre ubicacion)
    dia1 = datos['fecha1'].day
    dia2 = datos['fecha2'].day if datos['fecha2'] else dia1
    mes_nombre = obtener_mes_nombre(datos['fecha1'].month)
    texto_dias = f"{dia1} al {dia2}"
...
def generar_tipo_7_v2(datos):
    ...
    # 2. Caja Fecha Larga (Ancho Dinamico, Apilada sobre Ubicacion)
    dia1 = datos['fecha1'].day
    dia2 = datos['fecha2'].day if datos['fecha2'] else dia1
    mes_nombre = obtener_mes_nombre(datos['fecha1'].month)
    texto_dias = f"{dia1} al {dia2}"
...
def generar_tipo_7_v3(datos):
    ...
    # 1. Calculo Exacto Ubicacion (Izquierda) <-- INTERCAMBIO
    lugar = datos['lugar']
    s_lug = 61 if len(lugar) < 45 else 51 
    f_lugar = get_font("Canaro-Medium.ttf", s_lug)
    lines_loc = wrap_text_pixel(lugar, f_lugar, 800)
    ...
    # 2. Caja Fecha Larga (Izquierda, Ancho Dinamico, sobre ubicacion)
    dia1 = datos['fecha1'].day
    dia2 = datos['fecha2'].day if datos['fecha2'] else dia1
    mes_nombre = obtener_mes_nombre(datos['fecha1'].month)
    texto_dias = f"{dia1} al {dia2}"
    ...
    # 3. Logo Colaborador (Derecha Abajo) <-- INTERCAMBIO
    collab_img = None
    y_logo_collab_top = Y_BOTTOM_BASELINE
    if datos.get('logos') and len(datos['logos']) > 0:
        try:
...
def generar_tipo_8_v1(datos):
    ...
    # 1. LOGOS ARRIBA (Prefectura Izquierda, Firma Jota Derecha)
    margin_logos = 200
    if os.path.exists("flyer_logo.png"):
        logo = resize_por_alto(Image.open("flyer_logo.png").convert("RGBA"), 378)
        img.paste(logo, (margin_logos, 150), logo)
    ...
    # 2. Calculo Exacto Ubicacion (Izquierda Abajo) <-- INTERCAMBIO
    lugar = datos['lugar']
    s_lug = 61 if len(lugar) < 45 else 51 
    f_lugar = get_font("Canaro-Medium.ttf", s_lug)
    lines_loc = wrap_text_pixel(lugar, f_lugar, 800)
    ...
    # 3. Caja Fecha Larga (Izquierda, Ancho Dinamico, sobre ubicacion)
    dia1 = datos['fecha1'].day
    dia2 = datos['fecha2'].day if datos['fecha2'] else dia1
    mes_nombre = obtener_mes_nombre(datos['fecha1'].month)
    texto_dias = f"{dia1} al {dia2}"
    ...
    # 4. Logo Colaborador (Derecha Abajo) <-- REGLA APLICADA
    collab_img = None
    y_logo_collab_top = Y_BOTTOM_BASELINE
    if datos.get('logos') and len(datos['logos']) > 0:
        try:
    ...
    # 5. Descripcion 2 (Izquierda, Flotando sobre el bloque fecha)
    desc2 = datos.get('desc2', "")
    s_desc2 = 68
    f_desc2 = get_font("Canaro-Medium.ttf", s_desc2)
    lines_d2 = wrap_text_pixel(desc2, f_desc2, 800)
    ...
    # Dibujar Descripcion 2 (Izquierda, sobre la caja)
    if desc2:
        y_cursor_d2 = y_desc2_top + int(s_desc2 * 1.1)
        for l in lines_d2:
            dibujar_texto_sombra(draw, l, x_box, y_cursor_d2, f_desc2, offset=(4,4), anchor="ls")
...
def generar_tipo_8_v2(datos):
    ...
    # 4. Desc2 (Izquierda, sobre la caja de fecha) <-- CORRECCIÓN
    desc2 = datos.get('desc2', "")
    s_desc2 = 68
    f_desc2 = get_font("Canaro-Medium.ttf", s_desc2)
    lines_d2 = wrap_text_pixel(desc2, f_desc2, 900)
...
def generar_tipo_8_v3(datos):
    ...
    # 1. Calculo Exacto Ubicacion (Izquierda Abajo) <-- REGLA APLICADA
    lugar = datos['lugar']
    s_lug = 61 if len(lugar) < 45 else 51 
    f_lugar = get_font("Canaro-Medium.ttf", s_lug)
    lines_loc = wrap_text_pixel(lugar, f_lugar, 800)
    ...
    # 3. Logo Colaborador (Derecha Abajo) <-- REGLA APLICADA
    collab_img = None
    y_logo_collab_top = Y_BOTTOM_BASELINE
    if datos.get('logos') and len(datos['logos']) > 0:
        try:
    ...
    # 4. Descripcion 2 (Izquierda, Flotando sobre el bloque fecha)
    desc2 = datos.get('desc2', "")
    s_desc2 = 68
    f_desc2 = get_font("Canaro-Medium.ttf", s_desc2)
    lines_d2 = wrap_text_pixel(desc2, f_desc2, 800)
    ...
    # Dibujar Descripcion 2 (Izquierda, sobre el recuadro)
    if desc2:
        y_cursor_d2 = y_desc2_top + int(s_desc2 * 1.1)
        for l in lines_d2:
            dibujar_texto_sombra(draw, l, SIDE_MARGIN, y_cursor_d2, f_desc2, offset=(4,4), anchor="ls")
...
def generar_tipo_8_v4(datos):
    ...
    # 4. Descripcion 2 (Izquierda, sobre el recuadro fecha) <-- CORRECCION
    desc2 = datos.get('desc2', "")
    s_desc2 = 68
    f_desc2 = get_font("Canaro-Medium.ttf", s_desc2)
    lines_d2 = wrap_text_pixel(desc2, f_desc2, 900)
...
# ==============================================================================
# NUEVO: 11. GENERADORES DE PLANTILLAS TIPO 9 (1 Desc, 1 Fecha Cuadrada, 2 Collab)
# ==============================================================================
...
def generar_tipo_9_v1(datos):
    ...
    # 2. Caja Fecha Cuadrada (Izquierda, Apilada 100px sobre ubicacion)
    GAP_LOC_BOX = 100
    h_caja = 548
    w_caja = 548
    x_box = SIDE_MARGIN
    ...
    # 3. 2 Logos Colaboradores (Abajo Derecha, Nueva Regla Tipo 9)
    collab1 = None
    collab2 = None
    ...
    x_cursor = W - SIDE_MARGIN
    if collab2:
        x_cursor -= collab2.width
        img.paste(collab2, (x_cursor, Y_BOTTOM_BASELINE - collab2.height), collab2)
        x_cursor -= 50 # Espacio entre logos
    if collab1:
        x_cursor -= collab1.width
        img.paste(collab1, (x_cursor, Y_BOTTOM_BASELINE - collab1.height), collab1)
...
def generar_tipo_9_v2(datos):
    ...
    # 3. Calculo Exacto Ubicacion (Izquierda Abajo)
    lugar = datos['lugar']
    s_lug = 61 if len(lugar) < 45 else 51
    f_lugar = get_font("Canaro-Medium.ttf", s_lug)
    lines_loc = wrap_text_pixel(lugar, f_lugar, 1000)
    ...
    # 4. Calculo Caja (Apilada a 100px sobre ubicacion)
    GAP_LOC_BOX = 100
    h_caja = 548
    w_caja = 548
    x_box = SIDE_MARGIN
...
def generar_tipo_9_v3(datos):
    ...
    # 3. 2 Logos Colaboradores (Abajo Derecha, Regla Tipo 9)
    collab1 = None
    collab2 = None
    logos_list = datos.get('logos', [])
    if len(logos_list) > 0:
    ...
    if collab2:
        ...
        img.paste(collab2, (x_cursor, Y_BOTTOM_BASELINE - collab2.height), collab2)
        x_cursor -= 50 # Espacio entre logos
        max_h_collabs = max(max_h_collabs, collab2.height)
    if collab1:
        x_cursor -= collab1.width
        img.paste(collab1, (x_cursor, Y_BOTTOM_BASELINE - collab1.height), collab1)
        max_h_collabs = max(max_h_collabs, collab1.height)
    ...
    # 4. Descripcion 1 (DINAMICA, Izquierda) - BAJADO 30PX
    limit_y = y_box_top - 50 # Limita con la caja de fecha que esta abajo
    y_start_desc1 = y_titulo + 180 # Empezar 30px mas abajo
    max_h_desc1 = limit_y - y_start_desc1
...
# ==============================================================================
# 12. INTERFAZ DE USUARIO Y LOGICA PRINCIPAL ACTUALIZADA
# ==============================================================================
...
if not area_seleccionada:
    ...
    with col_der:
        ...
        # LOGICA MEJORADA: 2 UPLOADERS INDIVIDUALES
        st.markdown("<div class='label-negro'>LOGOS COLABORADORES (MÁXIMO 2)</div>", unsafe_allow_html=True)
        ...
        if st.button("✨ GENERAR FLYERS ✨", type="primary", use_container_width=True):
            ...
            if errores:
                ...
                # Guardar logos si fueron subidos
                if logo1:
                ...
                # Construir arreglo de rutas para mandar al motor
                rutas_logos = []
                if st.session_state.get('ruta_logo1') and os.path.exists(st.session_state['ruta_logo1']):
