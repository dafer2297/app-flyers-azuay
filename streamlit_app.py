...
def generar_tipo_2_v2(datos):
    ...
    # --- DESCRIPCIÓN 2 (ENCIMA DE FIRMA - CENTRADA - MOVIDA 60PX IZQ) ---
    desc2 = datos['desc2']
    if desc2 and firma:
        s_desc2 = 80
        path_medium = ruta_abs("Canaro-Medium.ttf")
        ...
        # Centro horizontal de la firma (MOVIDO 60 PX A LA IZQUIERDA)
...
def generar_tipo_2_v3(datos):
    ...
    # 3. Título "INVITA" (SUBIDO A 750)
    y_titulo = 750
    dibujar_texto_sombra(draw, "INVITA", SIDE_MARGIN, y_titulo, f_invita, offset=(10,10), anchor="lm")

    # 4. Descripción 1 (SUBIDA A 930)
    desc1 = datos['desc1']
    chars_desc = len(desc1)
    ...
    # --- DESCRIPCIÓN 2 (Debajo de Desc 1, encima de Fecha) ---
    desc2 = datos['desc2']
    if desc2:
        s_desc2 = 80
        try: f_desc2 = ImageFont.truetype(path_medium, s_desc2)
        ...
        y_cursor_d2 = y_desc + 40 # Margen pequeño tras Desc 1

        for line in lines_d2:
            if y_cursor_d2 < y_box: # Solo dibujar si no choca
                dibujar_texto_sombra(draw, line, SIDE_MARGIN, y_cursor_d2, f_desc2, offset=(5,5), anchor="ls")
                y_cursor_d2 += h_line_d2

    # 6. UBICACIÓN DERECHA ABAJO
    lugar = datos['lugar']
    s_lug = 72 if len(lugar) < 45 else 60
    try: f_lugar = ImageFont.truetype(ruta_abs("Canaro-Medium.ttf"), s_lug)
    except: f_lugar = ImageFont.load_default()
...
def generar_tipo_2_v4(datos):
    ...
    # Y_DESC SUBIDO A 960 (Antes 1070)
    y_desc = 960 
...
def generar_tipo_3_v1(datos):
    ...
    try:
        f_invita = ImageFont.truetype(ruta_abs("Canaro-Bold.ttf"), 220) 
        # Fuentes para la caja larga
        f_dias_largo = ImageFont.truetype(ruta_abs("Canaro-Black.ttf"), 140) # "20 al 24"
        f_mes_largo = ImageFont.truetype(ruta_abs("Canaro-Black.ttf"), 110)  # "DICIEMBRE"
    ...
    # 6. CAJA FECHA LARGA (TIPO 3)
    # Dimensiones fijas solicitadas
    h_caja = 352
    w_caja = 950 # Ancho ajustado para "20 al 24"
    x_box = SIDE_MARGIN
    y_box = Y_BOTTOM_BASELINE - 170 - h_caja # Misma línea base que T1 pero caja más pequeña en altura
    
    if os.path.exists("flyer_caja_larga.png"):
        # Usamos la imagen del recuadro largo
        caja = Image.open("flyer_caja_larga.png").convert("RGBA")
        # Forzamos altura 352, ancho proporcional o el que definimos
        caja = caja.resize((w_caja, h_caja), Image.Resampling.LANCZOS)
        img.paste(caja, (x_box, int(y_box)), caja)
        color_fecha = "white"
    else:
        # Dibujar rectángulo blanco si no existe imagen
        draw.rectangle([x_box, y_box, x_box+w_caja, y_box+h_caja], fill="white")
        color_fecha = "black"

    # Texto dentro de la caja larga
    cx = x_box + (w_caja / 2)
    cy = y_box + (h_caja / 2)
    
    dia1 = datos['fecha1'].day
    dia2 = datos['fecha2'].day if datos['fecha2'] else dia1
    mes_nombre = obtener_mes_nombre(datos['fecha1'].month) # "DICIEMBRE"
    
    texto_dias = f"{dia1} al {dia2}"
    
    # Dibujar "20 al 24" un poco más arriba del centro
    draw.text((cx, cy - 60), texto_dias, font=f_dias_largo, fill=color_fecha, anchor="mm")
    # Dibujar "DICIEMBRE" un poco más abajo
    ...
    with col_der:
        ...
        if st.button("✨ GENERAR FLYERS ✨", type="primary", use_container_width=True):
            ...
            if errores:
                ...
                if has_fecha2 and not has_desc2 and num_logos == 0:
                    generated_images['t3_v1'] = generar_tipo_3_v1(datos)
                    # Aquí añadiremos v2, v3, v4 cuando las creemos
                ...
                # (Por defecto si no es ninguno de los anteriores y no tiene logos)
                elif num_logos == 0:
    ...
    if 'datos_finales' not in st.session_state:
        ...
        with c_center:
            if generated:
                ...
                if tipo == 3:
                    order = ['t3_v1'] # Por ahora solo tenemos esta
                elif tipo == 2:
