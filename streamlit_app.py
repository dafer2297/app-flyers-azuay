...
# ==============================================================================
# 3. GENERADORES DE VARIANTES
# ==============================================================================

# --- VARIANTE 1: CLÁSICA (Tu código corregido) ---
def generar_tipo1_v1(datos):
    ...
    if fecha2:
        if os.path.exists("flyer_caja_fecha_larga.png"):
            caja = Image.open("flyer_caja_fecha_larga.png").convert("RGBA").resize((1000, 645), Image.Resampling.LANCZOS)
    ...
    if icon: img.paste(icon, (int(x_icon), int(y_icon)), icon)
    else: dibujar_texto_sombra(draw, "📍", x_icon + w_icon/2, y_icon + h_icon/2, f_lugar, anchor="mm")
...
# --- VARIANTE 2: MODERNA (Logo Centro, Fecha y Ubicación Izquierda) ---
def generar_tipo1_v2(datos):
    ...
    # --- CABECERA (SOLO LOGO PREFECTURA CENTRADO) ---
    if os.path.exists("flyer_logo.png"):
        logo = Image.open("flyer_logo.png").convert("RGBA")
        logo = resize_por_alto(logo, 378)
    ...
    y_base_txt = Y_BOTTOM_BASELINE
    x_txt_start = SIDE_MARGIN + 130 # Espacio para icono a la izquierda
    ...
    # --- ARRIBA DE UBICACIÓN: CAJA FECHA ---
    # Calculamos base de fecha para que no choque con ubicación
    ...
    if fecha2:
        ...
        draw.text((cx, cy + 170), obtener_mes_abbr(fecha1.month), font=f_mes_box, fill=color_fecha, anchor="mm")
        dia_sem = obtener_dia_semana(fecha1)
        dibujar_texto_sombra(draw, dia_sem, cx, y_linea_hora - 100, f_dia_semana, offset=(8,8), anchor="mm")
        dibujar_texto_sombra(draw, str_hora, cx, y_linea_hora, f_hora, offset=(8,8), anchor="mm")

    return img.convert("RGB")

def generar_flyer_automatico(datos):
    # Por defecto usa V1 si no se especifica variante
    return generar_tipo1_v1(datos)
...
if not area_seleccionada:
    ...
    with col_der:
        ...
        if archivo_subido:
            ...
            # Corrección de flujo para evitar error si no hay imagen en session_state
