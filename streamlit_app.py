...
# REGLA TIPO 9 (Alto max 300, Ancho max 600) - SE MANTIENE
...
def generar_tipo_2_v4(datos):
    ...
    # 4. Desc2 (Izquierda, sobre la caja fecha) <-- CORREGIDO AQUI
    desc2 = datos.get('desc2', "")
    s_desc2 = 68
    f_desc2 = get_font("Canaro-Medium.ttf", s_desc2)
    lines_d2 = wrap_text_pixel(desc2, f_desc2, 800)
...
def generar_tipo_6_v1(datos):
    ...
    # 1. LOGICA DE DISTRIBUCION DE LOS 3 LOGOS SUPERIORES (NUEVA REGLA 600x300)
    collab_img = None
    if datos.get('logos') and len(datos['logos']) > 0:
        try:
...
def generar_tipo_8_v1(datos):
    ...
    # 4. Logo Colaborador (Derecha Abajo) <-- INTERCAMBIO
    collab_img = None
    y_logo_collab_top = Y_BOTTOM_BASELINE
    if datos.get('logos') and len(datos['logos']) > 0:
        try:
...
# ==============================================================================
# 11. GENERADORES DE PLANTILLAS TIPO 9 (1 Desc, 1 Fecha, 2 Colaboradores)
# ==============================================================================
        ...
        if st.button("✨ GENERAR FLYERS ✨", type="primary", use_container_width=True):
            ...
            if errores:
                ...
                if num_lg == 2 and not fecha2 and not desc2:
                    generated = {'t9_v1': generar_tipo_9_v1(datos), 't9_v2': generar_tipo_9_v2(datos)}
                    tid = 9
