...
def generar_tipo_5_v1(datos):
    ...
    try:
        ...
        f_invita = ImageFont.load_default(); path_desc = None
    ...
    dibujar_texto_sombra(draw, "INVITAN", W/2, 780, f_invita, offset=(10,10))
    desc1 = datos['desc1']
    chars_desc = len(desc1)
    if chars_desc < 60: size_desc_val = 110; wrap_w = 35
    ...
    h_caja = 548; w_caja = 548; x_box = SIDE_MARGIN; cx = x_box + (w_caja / 2)
    y_logo_bottom = Y_BOTTOM_BASELINE
    y_logo = y_logo_bottom - collab_img.height if collab_img else y_logo_bottom
    
    if collab_img:
        img.paste(collab_img, (int(cx - (collab_img.width / 2)), int(y_logo)), collab_img)

    y_hora_txt = y_logo - 90; y_dia_txt = y_hora_txt - 72; y_box_bottom = y_dia_txt - 72; y_box = y_box_bottom - h_caja; cy = int(y_box + (h_caja / 2))

    if os.path.exists("flyer_caja_fecha.png"):
        caja = Image.open("flyer_caja_fecha.png").convert("RGBA").resize((w_caja, h_caja), Image.Resampling.LANCZOS)
        img.paste(caja, (x_box, int(y_box)), caja); color_fecha = "white"
    else:
        draw.rectangle([x_box, y_box, x_box+w_caja, y_box+h_caja], fill="white"); color_fecha = "black"
    ...
    str_hora = datos['hora1'].strftime('%H:%M %p')
    size_h = 93; size_h = 68 if datos['hora2'] else 93
    ...
    lugar = datos['lugar']; s_lug = 61 if len(lugar) < 45 else 51 
    try: f_lugar = ImageFont.truetype(ruta_abs("Canaro-Medium.ttf"), s_lug)
    except: f_lugar = ImageFont.load_default()
    ...
    curr_y = Y_BOTTOM_BASELINE - total_text_height + line_height
    for l in lines_loc:
        dibujar_texto_sombra(draw, l, x_text_start, curr_y, f_lugar, anchor="ls", offset=(3,3)); curr_y += line_height
    return img.convert("RGB")
...
def generar_tipo_5_v2(datos):
    ...
    if os.path.exists("flyer_sombra.png"):
        ...
        overlay = Image.new('RGBA', (W, H), (0,0,0,0)); d_over = ImageDraw.Draw(overlay)
        for y in range(int(H*0.3), H):
            alpha = int(255 * ((y - H*0.3)/(H*0.7))); d_over.line([(0,y), (W,y)], fill=(0,0,0, int(alpha*0.9)))
    ...
    try:
        ...
        path_extra = ruta_abs("Canaro-ExtraBold.ttf")
        if not os.path.exists(path_extra): path_extra = ruta_abs("Canaro-Black.ttf")
        f_dia_semana = ImageFont.truetype(path_extra, 110); path_desc = ruta_abs("Canaro-SemiBold.ttf")
    except:
        f_invita = f_dia_box = f_mes_box = f_dia_semana = ImageFont.load_default(); path_desc = None
    margin_logos_top = 300
    if os.path.exists("flyer_logo.png"):
        logo = Image.open("flyer_logo.png").convert("RGBA"); logo = resize_por_alto(logo, 378); img.paste(logo, (margin_logos_top, 150), logo)
    if datos.get('logos') and len(datos['logos']) > 0:
        try:
            collab_img = Image.open(datos['logos'][0]).convert("RGBA"); collab_img = redimensionar_logo_colaborador(collab_img)
            img.paste(collab_img, (int(W - margin_logos_top - collab_img.width), int(150 + (378 - collab_img.height) // 2)), collab_img)
        except Exception as e: pass
    dibujar_texto_sombra(draw, "INVITAN", W/2, 850, f_invita, offset=(10,10))
    desc1 = datos['desc1']; chars_desc = len(desc1)
    if chars_desc < 60: size_desc_val = 110; wrap_w = 35
    elif chars_desc < 120: size_desc_val = 90; wrap_w = 45
    elif chars_desc < 150: size_desc_val = 75; wrap_w = 55
    else: size_desc_val = 65; wrap_w = 65
    ...
    for line in textwrap.wrap(desc1, width=wrap_w):
        dibujar_texto_sombra(draw, line, W/2, y_desc, f_desc, offset=(8,8)); y_desc += int(size_desc_val * 1.1)
    if os.path.exists("flyer_firma.png"):
        firma = Image.open("flyer_firma.png").convert("RGBA"); firma = resize_por_alto(firma, 325); img.paste(firma, (W - firma.width - SIDE_MARGIN, int(Y_BOTTOM_BASELINE - firma.height + 50)), firma)
    lugar = datos['lugar']; s_lug = 72 if len(lugar) < 45 else 60
    try: f_lugar = ImageFont.truetype(ruta_abs("Canaro-Medium.ttf"), s_lug)
    except: f_lugar = ImageFont.load_default()
    lines_loc = textwrap.wrap(lugar, width=(20 if s_lug == 72 else 24)); line_height_loc = int(s_lug * 1.1); total_text_height = len(lines_loc) * line_height_loc; x_txt_start = SIDE_MARGIN + 130 
    if os.path.exists("flyer_icono_lugar.png"):
        icon = Image.open("flyer_icono_lugar.png").convert("RGBA"); icon = resize_por_alto(icon, h_icon := 260)
        img.paste(icon, (SIDE_MARGIN, int(Y_BOTTOM_BASELINE - (total_text_height/2) - (h_icon/2))), icon); x_txt_start = SIDE_MARGIN + icon.width + 30
    curr_y_loc = Y_BOTTOM_BASELINE - total_text_height + line_height_loc
    for l in lines_loc: dibujar_texto_sombra(draw, l, x_txt_start, curr_y_loc, f_lugar, anchor="ls", offset=(4,4)); curr_y_loc += line_height_loc
    y_linea_hora = Y_BOTTOM_BASELINE - total_text_height - 300; h_caja = 645; y_box = y_linea_hora - 170 - h_caja; x_box = SIDE_MARGIN; str_hora = datos['hora1'].strftime('%H:%M %p'); size_h = 110 if not datos['hora2'] else 80
    if datos['hora2']: str_hora += f" a {datos['hora2'].strftime('%H:%M %p')}"
    try: f_hora = ImageFont.truetype(path_extra, size_h)
    except: f_hora = ImageFont.load_default()
    if os.path.exists("flyer_caja_fecha.png"):
        caja = Image.open("flyer_caja_fecha.png").convert("RGBA").resize((645, 645), Image.Resampling.LANCZOS); img.paste(caja, (x_box, int(y_box)), caja); color_fecha = "white"
    else: draw.rectangle([x_box, y_box, x_box+645, y_box+645], fill="white"); color_fecha = "black"
    cx = x_box + (645 / 2); cy = int(y_box + (645 / 2)); draw.text((cx, cy - 50), str(datos['fecha1'].day), font=f_dia_box, fill=color_fecha, anchor="mm"); draw.text((cx, cy + 170), obtener_mes_abbr(datos['fecha1'].month), font=f_mes_box, fill=color_fecha, anchor="mm"); y_box_bottom = y_box + 645; dibujar_texto_sombra(draw, obtener_dia_semana(datos['fecha1']), cx, y_box_bottom + 85, f_dia_semana, offset=(8,8), anchor="mm"); dibujar_texto_sombra(draw, str_hora, cx, y_box_bottom + 170, f_hora, offset=(8,8), anchor="mm")
    return img.convert("RGB")
...
def generar_tipo_5_v3(datos):
    ...
    if os.path.exists("flyer_sombra.png"): sombra_img = Image.open("flyer_sombra.png").convert("RGBA").resize((W, H), Image.Resampling.LANCZOS); img.paste(sombra_img, (0, 0), sombra_img)
        ...
        overlay = Image.new('RGBA', (W, H), (0,0,0,0)); d_over = ImageDraw.Draw(overlay)
        for y in range(int(H*0.3), H): alpha = int(255 * ((y - H*0.3)/(H*0.7))); d_over.line([(0,y), (W,y)], fill=(0,0,0, int(alpha*0.9)))
        img = Image.alpha_composite(img, overlay); draw = ImageDraw.Draw(img)
    try:
        f_invita = ImageFont.truetype(ruta_abs("Canaro-Bold.ttf"), 220); path_desc = ruta_abs("Canaro-SemiBold.ttf"); f_dia_box = ImageFont.truetype(ruta_abs("Canaro-Black.ttf"), 297); f_mes_box = ImageFont.truetype(ruta_abs("Canaro-Black.ttf"), 170); path_extra = ruta_abs("Canaro-ExtraBold.ttf"); f_dia_semana = ImageFont.truetype(path_extra, 93)
    except: f_invita = f_dia_box = f_mes_box = f_dia_semana = ImageFont.load_default(); path_desc = None
    margin_logos = 200; img.paste(Image.open("flyer_logo.png").convert("RGBA").resize((int(Image.open("flyer_logo.png").width * (378/Image.open("flyer_logo.png").height)), 378), Image.Resampling.LANCZOS), (margin_logos, 150)) if os.path.exists("flyer_logo.png") else None; img.paste(Image.open("flyer_firma.png").convert("RGBA").resize((int(Image.open("flyer_firma.png").width * (325/Image.open("flyer_firma.png").height)), 325), Image.Resampling.LANCZOS), (W - int(Image.open("flyer_firma.png").width * (325/Image.open("flyer_firma.png").height)) - margin_logos, 170)) if os.path.exists("flyer_firma.png") else None
    dibujar_texto_sombra(draw, "INVITAN", SIDE_MARGIN, 780, f_invita, offset=(10,10), anchor="lm"); desc1 = datos['desc1']; chars_desc = len(desc1)
    if chars_desc < 50: s_desc = 130; wrap_w = 15
    elif chars_desc < 90: s_desc = 110; wrap_w = 18
    elif chars_desc < 130: s_desc = 85; wrap_w = 24
    else: s_desc = 70; wrap_w = 28
    f_desc = ImageFont.truetype(path_desc, s_desc) if path_desc and os.path.exists(path_desc) else ImageFont.load_default()
    y_desc = 940; [dibujar_texto_sombra(draw, l, SIDE_MARGIN, y_desc + i*int(s_desc*1.1), f_desc, offset=(8,8), anchor="ls") for i,l in enumerate(textwrap.wrap(desc1, width=wrap_w))]
    collab_img = redimensionar_logo_colaborador(Image.open(datos['logos'][0]).convert("RGBA")) if datos.get('logos') else None
    h_caja = 548; w_caja = 548; x_box = SIDE_MARGIN; cx = x_box + (w_caja / 2); y_logo = Y_BOTTOM_BASELINE - (collab_img.height if collab_img else 0)
    img.paste(collab_img, (int(cx - (collab_img.width / 2)), int(y_logo)), collab_img) if collab_img else None
    y_hora_txt = y_logo - 90; y_dia_txt = y_hora_txt - 72; y_box = y_dia_txt - 72 - h_caja; cy = int(y_box + (h_caja / 2))
    img.paste(Image.open("flyer_caja_fecha.png").convert("RGBA").resize((w_caja, h_caja), Image.Resampling.LANCZOS), (x_box, int(y_box))) if os.path.exists("flyer_caja_fecha.png") else draw.rectangle([x_box, y_box, x_box+w_caja, y_box+h_caja], fill="white")
    draw.text((cx, cy - 42), str(datos['fecha1'].day), font=f_dia_box, fill="white", anchor="mm"); draw.text((cx, cy + 144), obtener_mes_abbr(datos['fecha1'].month), font=f_mes_box, fill="white", anchor="mm"); str_hora = datos['hora1'].strftime('%H:%M %p') + (f" a {datos['hora2'].strftime('%H:%M %p')}" if datos['hora2'] else ""); f_hora = ImageFont.truetype(path_extra, 68 if datos['hora2'] else 93)
    dibujar_texto_sombra(draw, obtener_dia_semana(datos['fecha1']), cx, y_dia_txt, f_dia_semana, offset=(6,6), anchor="mm"); dibujar_texto_sombra(draw, str_hora, cx, y_hora_txt, f_hora, offset=(6,6), anchor="mm"); lugar = datos['lugar']; s_lug = 61 if len(lugar) < 45 else 51; f_lugar = ImageFont.truetype(ruta_abs("Canaro-Medium.ttf"), s_lug); lines_loc = textwrap.wrap(lugar, width=(24 if s_lug == 61 else 28)); x_text_start = W - SIDE_MARGIN - max([f_lugar.getlength(l) for l in lines_loc] or [200]); icon = resize_por_alto(Image.open("flyer_icono_lugar.png").convert("RGBA"), 221) if os.path.exists("flyer_icono_lugar.png") else None
...
def generar_tipo_5_v4(datos):
    # TIPO 5 - V4: Base T1_V4. Logos arriba (Pref+Collab). Fecha e Info REDUCIDOS al 85%. DISTANCIA 130px.
    fondo = datos['fondo'].copy(); W, H = 2400, 3000; SIDE_MARGIN = 180; Y_BOTTOM_BASELINE = H - 150; img = fondo.resize((W, H), Image.Resampling.LANCZOS).convert("RGBA"); draw = ImageDraw.Draw(img)
    if os.path.exists("flyer_sombra.png"): sombra_img = Image.open("flyer_sombra.png").convert("RGBA").resize((W, H), Image.Resampling.LANCZOS); img.paste(sombra_img, (0, 0), sombra_img)
    ...
    dibujar_texto_sombra(draw, "INVITAN", SIDE_MARGIN, 800, f_invita, offset=(10,10), anchor="lm"); desc1 = datos['desc1']; chars_desc = len(desc1); s_desc = 130 if chars_desc < 50 else (110 if chars_desc < 90 else (90 if chars_desc < 140 else 75)); wrap_w = 15 if chars_desc < 50 else (18 if chars_desc < 90 else (22 if chars_desc < 140 else 25)); f_desc = ImageFont.truetype(path_desc, s_desc) if path_desc else ImageFont.load_default(); [dibujar_texto_sombra(draw, l, SIDE_MARGIN, 800+70+s_desc + i*int(s_desc*1.1), f_desc, offset=(8,8), anchor="ls") for i,l in enumerate(textwrap.wrap(desc1, width=wrap_w))]
...
if not area_seleccionada:
    ...
    with col_izq:
        ...
        st.image(icono, width=350) if os.path.exists(icono) else None
        st.image("firma_jota.png", width=200) if os.path.exists("firma_jota.png") else None
    with col_der:
        ...
        st.markdown(f"<p style='text-align:right; color:black; font-size:12px;'>{len(desc1)+len(desc2)} / 175</p>", unsafe_allow_html=True)
        ...
        st.markdown("<div class='label-negro'>IMAGEN DE FONDO</div>", unsafe_allow_html=True)
        archivo_subido = st.file_uploader("img", type=['jpg','png','jpeg'])
        if archivo_subido:
            img_crop = st_cropper(Image.open(archivo_subido), realtime_update=True, aspect_ratio=(4,5), should_resize_image=False)
        ...
        if st.button("✨ GENERAR FLYERS ✨", type="primary", use_container_width=True):
            if not desc1 or not fecha1 or 'v_fondo' not in st.session_state: st.error("Faltan datos obligatorios")
            else:
                rutas_logos = []
                if logos:
                ...
                if num_lg >= 1 and not desc2 and not fecha2:
                ...
                elif desc2 and fecha2 and num_lg==0:
    ...
    if 'gen_imgs' not in st.session_state: st.warning("No hay datos"); st.rerun()
    c_l, c_c, c_r = st.columns([1.5, 3, 1.5])
    with c_l:
        st.image("mascota_pincel.png", width=350) if os.path.exists("mascota_pincel.png") else None
        st.image("firma_jota.png", width=280) if os.path.exists("firma_jota.png") else None
    with c_c:
        imgs = st.session_state.gen_imgs; sel = st.session_state.sel_var; st.image(imgs[sel], use_container_width=True)
        st.write(""); c_p, c_d, c_n = st.columns([1, 2, 1])
        vars_list = list(imgs.keys()); idx = vars_list.index(sel)
        with c_p:
            if st.button("⬅️"): st.session_state.sel_var = vars_list[(idx-1)%len(vars_list)]; st.rerun()
        with c_n:
            if st.button("➡️"): st.session_state.sel_var = vars_list[(idx+1)%len(vars_list)]; st.rerun()
        with c_d:
            buf = io.BytesIO(); imgs[sel].save(buf, format="PNG")
            if os.path.exists("mascota_final.png"):
                with open("mascota_final.png","rb") as f: m_b64 = base64.b64encode(f.read()).decode()
                st.markdown(f"<div style='text-align:center;'><a href='data:image/png;base64,{base64.b64encode(buf.getvalue()).decode()}' download='azuay.png'><img src='data:image/png;base64,{m_b64}' width='220' class='zoom-hover'><div style='color:white; font-weight:bold; margin-top:5px;'>DESCARGUE AQUI</div></a></div>", unsafe_allow_html=True)
            else: st.download_button("DESCARGAR", buf.getvalue(), "azuay.png", "image/png")
    st.write("---")
    cc1, cc2 = st.columns(2)
    with cc1:
        if st.button("✏️ MODIFICAR DATOS", use_container_width=True):
            st.query_params['area'] = st.session_state.get('v_area', 'Cultura'); st.rerun()
    with cc2:
